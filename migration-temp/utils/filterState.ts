...
export function getFilterStateFromSearchParams(searchParams: URLSearchParams): FilterState {
  ...
  const status = searchParams
    .getAll(`status`)
    .flatMap((status) => status.toLocaleLowerCase().split(`,`));
...
export function filterStateToSql(filterState: FilterState) {
  ...
  if (filterState.status?.length) {
    sqlWhere.push(`status IN (${filterState.status.map(() => `$${i++}`).join(` ,`)})`);
    sqlParams.push(...filterState.status);
  }
  if (filterState.priority?.length) {
    sqlWhere.push(`priority IN (${filterState.priority.map(() => `$${i++}`).join(` ,`)})`);
