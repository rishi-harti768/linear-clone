...
import { type FilterState, useFilterState } from '../utils/filterState';
...
export default function ({
  ...
  useEffect(() => {
    ...
    const checkFTSIndex = async () => {
      const res = await pg.query(`SELECT 1 FROM pg_indexes WHERE indexname = 'issue_search_idx';`);
      ...
      <div className="flex justify-between flex-shrink-0 pl-2 pr-6 border-b border-gray-200 h-14 lg:pl-9">
        ...
        <div className="flex items-center">
          ...
          <span>
            ...
            {totalIssuesCount !== undefined && filteredIssuesCount !== totalIssuesCount
        ...
        <div className="flex flex-shrink-0 pl-2 pr-6 border-b border-gray-200 lg:pl-9 py-2">
            ...
            <div className="flex pr-4 space-x-[1px]">
              ...
              <span className="px-1 bg-gray-300 ">
                {filterState.priority
                  ?.map((priority) => PriorityDisplay[priority as keyof typeof PriorityDisplay])
                  .join(`, `)}
              </span>
              <span
          ...
          {!!filterState.status?.length && (
            <div className="flex pr-4 space-x-[1px]">
              ...
              <span className="px-1 bg-gray-300 ">
                {filterState.status
                  ?.map((status) => StatusDisplay[status as keyof typeof StatusDisplay])
                  .join(`, `)}
              </span>
              <span
                className="px-1 bg-gray-300 rounded-r cursor-pointer flex items-center"
